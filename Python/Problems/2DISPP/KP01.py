...
def dimensions(dataset: str):   
    ...
    specs = {
        ...
        'jackobs1':  (40.0,    13.0,    1, [0,1,2,3]),
        'jackobs2':  (70.0,    28.2,    1, [0,1,2,3]),
        'shapes0':   (63.0,    40.0,    1, [0]),
        'shapes1':   (40.0,    59.0,    1, [0,2]),
        'shapes2':   (27.3,    15.0,    1, [0,2]),
        'dighe1':    (138.14,  100.0,    1, [0]),
        'dighe2':    (100.0,  134.05,    1, [0]),
...
class SPP2D():
    def __init__(self,dataset='fu',Base=None,Altura=None,Escala=None, Graus = None, tabela = None, margem = 0, tempo=200, decoder = 'D1'):
        ...
        self.lista_original = []
        for pol in lista:
            if pol not in self.lista_original:
                self.lista_original.append(pol)
        
        
        self.lista = copy.deepcopy(self.lista_original)
        self.max_pecas = int((1.5*self.area) / np.mean([Polygon(p).area for p in self.lista]))
    ...
    def decoder(self, keys):
        ...
        if self.decoder_type == 'D0':
            ...
            pieces_idx = []
            pecas_tipos = len(self.lista)
            for key in pieces:
                pieces_idx.append(int(key * pecas_tipos))
    ...
    def cost(self, sol, tag = 0, save  =True):
        if self.decoder_type == 'D0':
            ...
            if tuple(sol) in self.dict_sol:
                ...
                for idx in pieces:
                    # print(i)
                    # posic = len(self.pecas_posicionadas)
                    # pecas_validas = []
                    # for peca in self.lista_original:
                    #     if self.feasible(self.lista.index(peca), rot[i]):
                    #         pecas_validas.append(peca)
                    # index = int(idx * len(pecas_validas))
                    self.pack(self.lista.index(self.lista_original[idx]),rot[i], regras[i])
                    # if len(self.pecas_posicionadas) == posic:
                    #     break
                    i+=1
                    # self.plot()
    ...
    def plot(self, legenda):
        draw_cutting_area(self.pecas_posicionadas, self.base, self.altura ,legenda=legenda, filename=f'C:\\Users\\felip\\Documents\\GitHub\\RKO\\Python\\Images\\KP01\\{self.instance_name}\\{self.instance_name}_{time.time()}.png')
    ...
    def area_usada(self):
        ...
        return round((area / area_bin) * 100, 2) 
