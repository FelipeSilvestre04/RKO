...
class SPP2D():
    def __init__(self,dataset='fu',Base=None,Altura=None,Escala=None, Graus = None, tabela = None, margem = 0, tempo=200, decoder = 'D1', pairwise = False):
        ...
        elif os.path.exists(f"nfp_{self.dataset}_ssp0.txt") and  (not pairwise or porcentagem ==0.0):
            with open(f"nfp_{self.dataset}_ssp0.txt", "r") as f:
                conteudo = f.read()
...
if __name__ == '__main__':
    instancias = ["jackobs1","swim","jackobs2"]    
            ...
            for restart in [1/6]:                
                for ins in instancias:
                    for decoder in decoders:
                        ...
                        env = SPP2D(dataset=ins, tempo=tempo * restart, decoder=decoder, pairwise=False)
                        i = 0
                        start = time.time()
