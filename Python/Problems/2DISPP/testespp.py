...
class SPP2D():
    ...
    def plot_pairwise_geometries(self, peca1_poly, peca2_poly, nfp_poly, titulo=""):
        """
        Plota as geometrias envolvidas na análise de um par de peças.

        Args:
            peca1_poly (Polygon): Polígono da primeira peça (fixa).
            peca2_poly (Polygon): Polígono da segunda peça (móvel, já posicionada).
            nfp_poly (Polygon): O No-Fit Polygon entre as duas peças.
        ...
        plt.show()
    def pairwise(self, porcent):
        # ATENÇÃO: Limitei os loops para gerar poucos plots. 
        # Modifique os ranges para testar os pares que você deseja analisar.
        # Ex: i in [2], j in [5]
        for i in range(1): # Analisa a peça de índice 0
            for j in range(i + 6, 10): # Analisa a peça de índice 1
                for grau1_idx in [self.graus[0]]: # Apenas a primeira rotação
                    for grau2_idx in [self.graus[0]]: # Apenas a primeira rotação
                        
                        # --- Preparação das Geometrias ---
                        peca1_coords = self.lista_original[i]
                        peca2_coords = self.lista_original[j]
                        
                        # Usa o seu método rot_pol, que opera sobre self.lista
                        # Garantimos que self.lista está completo antes de chamar
                        self.reset() 
                        peca1_rot_coords = self.rot_pol(i, grau1_idx)
                        poly1 = Polygon(peca1_rot_coords)

                        # --- Cálculo do NFP e dos Pontos de Interseção ---
                        self.acao(i, 0, 0, grau1_idx)
                        nfp_result, intersec_result = self.nfp(self.lista.index(peca2_coords), grau2_idx)
                        self.remover_ultima_acao()

                        if not nfp_result or nfp_result.is_empty:
                            print(f"NFP não pôde ser calculado para o par ({i},{j}) com rotações ({grau1_idx},{grau2_idx})")
                            continue

                        # --- Coleta de TODOS os Pontos Candidatos ---
                        pontos_candidatos = list(nfp_result.exterior.coords)
                        if intersec_result and not intersec_result.is_empty:
                            pontos_de_intersecao = extrair_vertices(intersec_result)
                            pontos_candidatos.extend(pontos_de_intersecao)
                        
                        pontos_candidatos = list(dict.fromkeys(pontos_candidatos))
                        print(f"Encontrados {len(pontos_candidatos)} pontos de encaixe para o par ({i},{j}).")

                        # --- Loop de Visualização e Cálculo ---
                        for ponto_encaixe in pontos_candidatos:
                            
                            self.reset() # Garante que self.lista está completo para rot_pol
                            peca2_rot_coords = self.rot_pol(j, grau2_idx)
                            peca2_transladada_coords = [(p[0] + ponto_encaixe[0], p[1] + ponto_encaixe[1]) for p in peca2_rot_coords]
                            poly2 = Polygon(peca2_transladada_coords)
                            
                            # --- INÍCIO: CÁLCULO DOS CRITÉRIOS DE PAIRWISE ---
                            ...
                            # "Vazios" do fecho convexo (Convex Hull Vacancies)
                            chv1 = ch_poly1.difference(poly1)
                            chv2 = ch_poly2.difference(poly2)

                            # [cite_start]Critério 1 (Cr1): Mede o preenchimento dos vazios [cite: 301]
                            intersecao_p2_em_chv1 = poly2.intersection(chv1).area
                            intersecao_p1_em_chv2 = poly1.intersection(chv2).area
                            
                            ratio1 = intersecao_p2_em_chv1 / chv1.area if chv1.area > 1e-9 else 0
                            ratio2 = intersecao_p1_em_chv2 / chv2.area if chv2.area > 1e-9 else 0
                            cr1 = max(ratio1, ratio2)

                            # [cite_start]Critério 2 (Cr2): Mede a utilização da área combinada [cite: 303]
                            cr2 = 0
                            if ch_uniao.area > 1e-9:
                                cr2 = (poly1.area + poly2.area) / ch_uniao.area

                            # Valor final do cluster para este ponto de encaixe
                            cluster_value = cr1 + cr2
                            
                            # --- FIM: CÁLCULO DOS CRITÉRIOS DE PAIRWISE ---

                            # Título atualizado para incluir os valores calculados
                            titulo = (f"Par (Peça {i}, G {grau1_idx}) vs (Peça {j}, G {grau2_idx})\n"
                                    f"Ponto: ({ponto_encaixe[0]:.1f}, {ponto_encaixe[1]:.1f}) | "
                                    f"Cr1: {cr1:.3f}, Cr2: {cr2:.3f} | ClusterValue: {cluster_value:.3f}")

                            # Chama a função de plotagem dedicada
                            self.plot_pairwise_geometries(poly1, poly2, nfp_result, titulo)
                            
                            # input("Pressione Enter para ver o próximo ponto de encaixe...")

        return # A função agora serve apenas para plotar
...
if __name__ == '__main__':
    ...
    instancias = ["swim","jackobs2","fu","jackobs1","trousers", "jackobs2","shapes0","shapes1","shapes2","albano","shirts","dighe1","dighe2","dagli","mao","marques","swim"] 
